...
import com.meteor.chat.common.enums.DeleteStatusEunm;
...
import com.meteor.chat.msg.domain.vo.ChatMessageReq;
...
public class MsgAdapter {
     ...
     * @return
     */
    public static MsgReadInfoDTO buildMsgReadInfoDTO(Message msg, List<ContactInfoDTO> contacts) {
    ...
    public static ChatMessageReq buildApprovalMsg(Long roomId, String content) {
        ...
        textMsgReq.setContent(StrUtil.isBlank(content) ? CommonConstants.USER_APPROVAL_MSG_CONTENT : content);
    ...
    public static ChatMessageReq buildInviteSuccessMsg(Long roomId, String inviterName, List<String> inviteeNameList) {
        // 拼接系统消息的内容
        StringBuilder content = new StringBuilder();
        content.append(String.format("\"%s\"", inviterName));
        content.append(" 邀请 ");
        for(String name
                : inviteeNameList) {
        ...
        return MsgAdapter.buildMemberChange(roomId, content.toString());
    }
